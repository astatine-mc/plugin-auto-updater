...
프록시뿐만 아니라 **로비, 야생, 던전 등 여러 개의 백엔드 버킷(Paper) 서버를 한 곳에서 중앙 관리**할 수 있습니다.
...
설정은 [예시 설정](examples/config.yml)을 `Proxy/plugins/pluginautoupdater/config.yml`로 복사한 뒤 YAML 형식으로 서버와 플러그인을 등록합니다.
...
# platform은 proxy는 velocity, 그 외는 기본 paper로 자동 지정됩니다.
# ==============================================================================
servers:
  proxy:
    directory: "."
...
# ==============================================================================
plugins:
  # [예시 1] 여러 버킷 서버에 동시 적용 (servers에 목록 지정)
  ...
  # [예시 2] 특정 버킷 서버에만 적용 (server: lobby)
  # viaversion:
  #   enabled: true
  #   server: lobby
  #   source: modrinth
  ...
  # [예시 3] 프록시 서버 전용 플러그인 (server: proxy)
  # tab:
  #   enabled: true
  #   server: proxy
  #   source: modrinth
...
- `servers`: 관리할 서버(버킷) 목록을 정의합니다. (예: `proxy`, `lobby`, `survival`, `dungeon` 등)
- `servers` 또는 `server`: 플러그인이 적용될 대상 서버를 지정합니다. `[lobby, survival]` 형태로 여러 서버를 지정하면 각 서버의 `update/` 폴더에 각각 자동으로 스테이징됩니다.
- `target-file`: (선택 사항) 생략 시 각 서버 `plugins/` 폴더의 기존 파일명을 자동 감지하여 **업데이트 이전의 파일명 그대로 덮어씌웁니다.**
- `source`:
...
1. 새 버킷 서버(예: `survival`)를 만듭니다.
2. `scripts/apply-pending-updates.sh`를 `survival/plugin-updater/apply-pending-updates.sh`로 복사합니다.
3. `survival/start.sh`의 Java 실행 직전에 `"$SCRIPT_DIR/plugin-updater/apply-pending-updates.sh" "$SCRIPT_DIR"`를 호출하도록 추가합니다.
4. 프록시의 `config.yml`의 `servers:` 섹션에 `survival: "../survival"`을 한 줄 추가합니다.

## 명령어

- `/pluginupdater check` (또는 `/updater check`): 등록된 모든 플러그인의 최신 버전을 즉시 비동기로 조회하고 필요한 경우 각 서버의 `update/` 폴더에 스테이징합니다.
- `/pluginupdater reload` (또는 `/updater reload`): `config.yml` 설정을 다시 불러옵니다.
- `/pluginupdater status` (또는 `/updater status`): 등록된 서버 목록 및 플러그인 규칙 상태를 확인합니다.
...
2. Modrinth(제공 SHA-512 검증) 또는 LuckPerms 전용 API에서 최신 JAR을 각 서버의 `update/` 폴더에 내려받고 플러그인 메타데이터(`plugin.yml` 또는 `velocity-plugin.json`)를 확인합니다.
3. 각 서버의 `update/pending.tsv`에 대기 정보를 기록합니다. **실행 중인 활성 `plugins/` 폴더는 이 시점에 절대 변경하지 않습니다.**
4. 각 서버가 재시작될 때 해당 서버의 `start.sh`가 Java 실행 직전에 `apply-pending-updates.sh`를 호출하여 SHA-512를 재확인하고, 기존 활성 JAR을 `update-backup/<UTC-배치>/`로 백업한 뒤 새 JAR을 이동 적용합니다.
